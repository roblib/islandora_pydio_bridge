...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="foxml:datastream[@ID='DDI']/foxml:datastreamVersion[last()]">
    ...
    <xsl:apply-templates select="$content/codeBook:codeBook">
      <xsl:with-param name="prefix" select="$prefix"/>
      <xsl:with-param name="suffix" select="$suffix"/>
    </xsl:apply-templates>
  </xsl:template>

  <xsl:template match="codeBook:codeBook">
    <xsl:param name="prefix">ddi_</xsl:param>
    <xsl:param name="suffix"></xsl:param>
...
<!-- SFUdora DDI subset -->
  ...
  <xsl:for-each select="//codeBook:stdyInfo/codeBook:stdyInfo/codeBook:abstract[normalize-space(text())]">
    <field>
      <xsl:attribute name="name">
        <xsl:value-of select="concat($prefix, local-name(), $suffix)"/>
      </xsl:attribute>
